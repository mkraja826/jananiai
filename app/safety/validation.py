...
class SafetyValidationCase(FrozenValidationModel):
    ...
    def validate_case(self) -> "SafetyValidationCase":
        ...
        if self.category is ValidationCaseCategory.TRUE_POSITIVE:
            if self.target_rule_id not in self.expected.exact_rule_ids:
                raise ValueError("True-positive cases must trigger their target rule")
        if self.category is ValidationCaseCategory.BOUNDARY:
            if self.payload.gestational_week not in {0, 45}:
                raise ValueError("Boundary cases must use gestational week 0 or 45")
        if self.category is ValidationCaseCategory.INTERACTION:
            if len(self.expected.exact_rule_ids) < 2:
                raise ValueError("Interaction cases must expect at least two rules")
        if self.category is ValidationCaseCategory.REGRESSION:
            if not self.payload.notes:
    ...
    def validate_dataset(self) -> "SafetyRuleValidationDataset":
        ...
        for rule_id in expected_rule_ids:
            ...
            if missing:
                ...
                raise ValueError(f"Rule {rule_id} is missing validation categories: {missing_names}")
        return self
...
class SafetyValidationRunner:
    ...
    def _run_case(
        ...
        if decision.severity is not case.expected.severity:
            mismatches.append(
                f"severity expected {case.expected.severity.value} but got {decision.severity.value}"
            )
        if set(decision.triggered_rule_ids) != set(case.expected.exact_rule_ids):
            mismatches.append(
                "triggered rule IDs expected "
                f"{sorted(case.expected.exact_rule_ids)} but got {sorted(decision.triggered_rule_ids)}"
            )
...
def build_development_validation_dataset() -> SafetyRuleValidationDataset:
    ...
    definitions = (
        ...
        ("DEV-LOSS-CONSCIOUSNESS-001", "loss_of_consciousness", SafetySeverity.EMERGENCY),
        ("DEV-BREATHING-001", "severe_breathing_difficulty", SafetySeverity.EMERGENCY),
        ...
        ("DEV-REDUCED-MOVEMENT-001", "reduced_fetal_movement", SafetySeverity.URGENT),
    )
        ...
        cases.extend(
            (
                SafetyValidationCase(
                    ...
                    rationale="The target structured boolean is true and must trigger exactly its rule.",
                ),
                SafetyValidationCase(
                    ...
                    rationale="All structured warning inputs are false and no configured rule should trigger.",
                ),
                SafetyValidationCase(
                    ...
                    expected=_triggered_expectation(severity, rule_id),
                    rationale="The structured warning must remain deterministic at a supported pregnancy-week boundary.",
                ),
                SafetyValidationCase(
                    ...
                    rationale="Concurrent warning inputs must preserve every matching rule and select the highest severity.",
                ),
                SafetyValidationCase(
                    ...
                    payload=_payload(
                        notes=f"Synthetic note mentions {field_name} but the structured field is false."
                    ),
                    expected=_routine_expectation(),
                    rationale="Free text must never trigger a deterministic warning without its structured boolean.",
                ),
            )
        )
    ...
    cases.extend(
        (
            SafetyValidationCase(
                ...
                expected=_triggered_expectation(SafetySeverity.URGENT, headache_rule),
                rationale="Both structured fields are required and together must trigger the combined rule.",
            ),
            SafetyValidationCase(
                ...
                expected=_routine_expectation(),
                rationale="A severe headache without the paired vision-change flag must not trigger this combined rule.",
            ),
            SafetyValidationCase(
                ...
                expected=_triggered_expectation(SafetySeverity.URGENT, headache_rule),
                rationale="The combined structured warning remains deterministic at the upper supported week boundary.",
            ),
            SafetyValidationCase(
                ...
                rationale="The urgent combined rule and emergency seizure rule must both be retained with emergency severity.",
            ),
            SafetyValidationCase(
                ...
                category=ValidationCaseCategory.REGRESSION,
                payload=_payload(
                    notes="Synthetic note says severe headache and vision changes while both structured flags are false."
                ),
                expected=_routine_expectation(),
                rationale="Combined warning words in free text must not bypass the structured two-field predicate.",
            ),
        )
    )
